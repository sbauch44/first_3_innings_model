...
def run_incremental_update_and_feature_recalc():
    ...
    try:
        ...
        logging.error(f"Error fetching Statcast data for {yesterday_str}: {e}", exc_info=True) # Log error with traceback
    ...
    try:
        ...
        logging.info(f"Loaded {df_historical_pa_helpers.shape[0]} historical PA records.")
        ...
        df_pa_full_updated = df_new_pa_helpers # Start fresh if historical doesn't exist
    ...
    try:
        logging.info(f"Saving updated historical PA data to: {historical_pa_helpers_path}")
    ...
    try:
        ...
        logging.info(f"filtering dataframe to only include relevant columns...")
    ...
    try:
        ...
        logging.error(f"Error fetching park factors data for {yesterday_str}: {e}", exc_info=True) # Log error with traceback
        return False # Indicate failure
    ...
    try:
        ...
        logging.info(f"Saving transformed park factors data to: {final_park_factors_path}")
    ...
    try:
        ...
        logging.error(f"Error fetching defensive stats data for {yesterday_str}: {e}", exc_info=True)
    ...
    if defensive_stats_df is None:
        logging.warning("No defensive stats DataFrame to process. Skipping transformation and save.")
        return True
    try:
        ...
        final_defensive_stats = data_processor.calculate_cumulative_defensive_stats(defensive_stats_df, defensive_total_innings)
        
        logging.info(f"Saving transformed defensive stats data to: {final_defensive_stats_path}")
    ...
    try:
        ...
        if fangraphs_bat_proj_df is None or fangraphs_bat_proj_df.is_empty():
            ...
            logging.info(f"Fetched {fangraphs_bat_proj_df.shape[0]} new Fangraphs batter projections.")
    except Exception as e:  
        logging.error(f"Error fetching Fangraphs batter projections: {e}", exc_info=True)
        return False # Indicate failure
    ...
    try:
        ...
        if fangraphs_pit_proj_df is None or fangraphs_pit_proj_df.is_empty():
            ...
            logging.info(f"Fetched {fangraphs_pit_proj_df.shape[0]} new Fangraphs pitching projections.")
    except Exception as e:
        logging.error(f"Error fetching Fangraphs pitching projections: {e}", exc_info=True)
        return False

    # --- 14. Store Fangraphs Projections ---
    final_bat_projections_path = f"{config.BASE_FILE_PATH}fangraphs_bat_projections.parquet"
    try:      
        logging.info(f"Saving fangraphs batter projections to: {final_bat_projections_path}")
        fangraphs_bat_proj_df.write_parquet(final_bat_projections_path)
    except Exception as e:
    ...
    try:      
        logging.info(f"Saving fangraphs pitcher projections to: {final_pit_projections_path}")
        fangraphs_pit_proj_df.write_parquet(final_pit_projections_path)
    except Exception as e:
    ...
    for game in schedule:
        try:
            ...
            if not game_pk or not game_start_str or not isinstance(game_pk, int):
                 logging.warning(f"  Skipping game due to missing pk or start time: {game}")
                 skipped_count += 1
                 continue
            if 'tbd' in status or 'postponed' in status or 'cancelled' in status or 'suspended' in status:
            ...
            game_start_utc = datetime.fromisoformat(game_start_str.replace('Z', '+00:00')).replace(tzinfo=pytz.utc)
            ...
            if trigger_time_utc < datetime.now(pytz.utc):
                 logging.info(f"  Skipping game {game_pk} - Trigger time {trigger_time_utc} is in the past.")
                 skipped_count += 1
                 continue

            logging.info(f"  Scheduling trigger for game {game_pk} at {trigger_time_utc} UTC")
            scheduled_count += 1
            ...
            logging.error(f"Error processing/scheduling game {game.get('game_id', 'N/A')}: {e}", exc_info=True)
            skipped_count += 1

    logging.info(f"Attempted to schedule triggers for {scheduled_count} games. Skipped {skipped_count} games.")
...
if __name__ == "__main__":
    ...
    if update_success:
        ...
        logging.error("Halting daily process due to error during data update/recalculation.")

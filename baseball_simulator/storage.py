...
def save_dataframe(df: pl.DataFrame, file_name_with_ext: str, sub_path: str = ""):
    ...
    try:
        ...
        logger.error(
            f"Error saving DataFrame to {full_path_obj if full_path_obj else file_name_with_ext}: {e}",
            exc_info=True,
        )
...
def load_dataframe(file_name_with_ext: str, sub_path: str = "") -> pl.DataFrame | None:
    ...
    try:
        ...
        logger.error(
            f"Configuration error (e.g., BASE_FILE_PATH not set in config): {ae}",
            exc_info=True,
        )
        return None

    logger.info(f"Attempting to load DataFrame from: {full_path_obj}")
    if not full_path_obj.exists() or not full_path_obj.is_file():
        logger.warning(
            f"File not found or is not a file at {full_path_obj}. Returning None."
        )
        return None
    try:
        ...
        logger.info("Successfully loaded DataFrame from %s", full_path_obj)
        return df_to_load
...
def save_simulation_results(df: pl.DataFrame, date_str: str, game_pk: int):
    ...
    save_dataframe(
        df, file_name, sub_path=str(results_sub_path)
    )  # Convert Path to str for sub_path arg
    ...
    if results_path_obj.exists() and results_path_obj.is_dir():
        ...
        for file_path_obj in results_path_obj.iterdir():  # Iterate over Path objects
            if file_path_obj.is_file() and file_path_obj.suffix == ".parquet":
                ...
                df_temp = load_dataframe(
                    file_path_obj.name, sub_path=str(relative_sub_path)
                )
                if df_temp is not None:
                    all_dfs.append(df_temp)
        if all_dfs:
            logging.info(
                f"Concatenating {len(all_dfs)} result DataFrames for date {date_str}."
            )
            return pl.concat(all_dfs, how="vertical_relaxed")
    logging.warning(
        f"No simulation result files found in directory: {results_path_obj}"
    )
    return None
